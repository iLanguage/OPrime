if (OPrime) {
  OPrime.debug("Loading EncryptionServices");
} else {
  console.log("Loading EncryptionServices");
}

angular.module('EncryptionServices', [ 'ngResource' ])
/**
 ...
 *          views can choose to not display the entire string for the user.
 */
.factory("EncryptValue", function(contents) {
  var result = CryptoJS.AES.encrypt(contents, OPrime.contentEncryptionToken());
  // return the base64 version to save it as a string in the corpus
  return "confidential:" + btoa(result);
})
/**
...
.factory(
    ...
    function(encrypted) {
      ...
      encrypted = atob(encrypted);
      resultpromise = CryptoJS.AES.decrypt(encrypted,
          OPrime.contentEncryptionToken()).toString(CryptoJS.enc.Utf8);
      return resultpromise;
    });
